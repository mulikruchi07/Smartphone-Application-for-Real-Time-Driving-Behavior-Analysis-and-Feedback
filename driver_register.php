...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check if email already exists in driver_details
    ...
    // Check car registration number in cars table (not owner_details)
    $checkCarRegQuery = "SELECT * FROM cars WHERE registration_number = ?";
    ...
    if ($carRegResult->num_rows > 0) {
        ...
        // Insert driver details into the driver_details table
        ...
        if ($stmt->execute()) {
            // Start session and save driver details
            session_start();
        ...
        echo "<script>alert('Car registration number not found in cars records.'); window.location.href='Driver_regis.html';</script>";
    }
}
